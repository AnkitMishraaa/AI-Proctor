...
gaze = GazeTracking()
webcam = cv2.VideoCapture("sm5.mp4")
...
while True:
    ...
    if sec == secs:
        ...
        center_nose = (image_points[0][0], image_points[0][1])
        end_nose_x = nose_end_point2D[0][0][0]
        end_nose_y = nose_end_point2D[0][0][1]
        away_from_center = round(math.dist(center, center_nose))
        secs= secs+1
        #data_list.append([left_pupil, right_pupil, center_x, center_y, p1, p2, values])
           

        if 0 < end_nose_x < center_nose_x and 0 < end_nose_y < center_nose_y:
            print("Looking at 1st")
            cv2.putText(frame, "You are looking on 1st grid", (10, 100), cv2.FONT_HERSHEY_DUPLEX, 0.9, (120, 100, 60), 1)
            data_list.append(['1st', left_pupil, right_pupil, center_x, center_y, p1, p2, away_from_center,secs])
        
        elif center_nose_x < end_nose_x < (w*10) and 0 < end_nose_y < center_nose_y:
            print("looking at 2nd")
            cv2.putText(frame, "You are looking on 2nd grid", (10, 100), cv2.FONT_HERSHEY_DUPLEX, 0.9, (120, 100, 60), 1)
            data_list.append(['2nd', left_pupil, right_pupil, center_x, center_y, p1, p2, away_from_center,secs])

        elif 0 < end_nose_x < center_nose_x and center_nose_y < end_nose_y < (h*10):
            print("looking at 4th")
            cv2.putText(frame, "You are looking on 4th grid", (10, 100), cv2.FONT_HERSHEY_DUPLEX, 0.9, (120, 100, 60), 1)
            data_list.append(['4th', left_pupil, right_pupil, center_x, center_y, p1, p2, away_from_center,secs])

        else:
            print("looking at 3rd")
            cv2.putText(frame, "You are looking on 3rd grid", (10, 100), cv2.FONT_HERSHEY_DUPLEX, 0.9, (120, 100, 60), 1)
            data_list.append(['3rd', left_pupil, right_pupil, center_x, center_y, p1, p2, away_from_center,secs])
        ...
        cv2.putText(frame, "Left pupil:  " + str(left_pupil), (420, 30), cv2.FONT_HERSHEY_DUPLEX, 0.9, (120, 100, 60), 1)
        cv2.putText(frame, "Right pupil: " + str(right_pupil), (420, 70), cv2.FONT_HERSHEY_DUPLEX, 0.9, (120, 100, 60), 1)
        
        
        #values = round(math.dist(center, center_nose))
        ...
        df = pd.DataFrame(data_list, columns=['quadrant','left_pupil','right_pupil','gaze_center_x', 'gaze_center_y', 'nose_end_points',
                                      'gaze_end_points','away_from_center','secs'])                              
        
        
        #df["away_from_center"] = values
